...
export const tableService = {
  async getTables(): Promise<any[]> {
    ...
    // Fetch tables
    let tables = await Table.find({}).sort({ table_number: 1 }).lean();
    
    // Auto-create missing tables up to 12 if count is low
    if (tables.length < 12) {
      const existingNumbers = new Set(tables.map(t => t.table_number));
      const missingTables = [];
      for (let i = 1; i <= 12; i++) {
        if (!existingNumbers.has(i)) {
          missingTables.push({ table_number: i, status: 'free' });
        }
      }
      if (missingTables.length > 0) {
        await Table.insertMany(missingTables);
        tables = await Table.find({}).sort({ table_number: 1 }).lean();
      }
    }
