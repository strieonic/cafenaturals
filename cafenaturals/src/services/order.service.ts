...
export const orderService = {
  async getActiveOrder(tableId: string): Promise<any | null> {
    ...
    return {
      ...
      timer_start: order.timer_start ? order.timer_start.toISOString() : null,
      timer_charge: order.timer_charge || 0,
      timer_duration_seconds: order.timer_duration_seconds || 0,
  ...
  async startTimer(orderId: string): Promise<string> {
    await dbConnect();
    const now = new Date();
    await Order.updateOne({ _id: orderId }, { timer_start: now });
    return now.toISOString();
  },

  async stopTimer(orderId: string, finalCharge: number = 0, reset: boolean = false): Promise<void> {
    await dbConnect();
    const order = await Order.findById(orderId);
    if (!order) return;
    ...
    if (order.timer_start) {
      additionalSeconds = Math.floor((Date.now() - new Date(order.timer_start).getTime()) / 1000);
    }
    
    const newDuration = reset ? 0 : (order.timer_duration_seconds || 0) + additionalSeconds;
    
    await Order.updateOne({ _id: orderId }, { 
      timer_start: null, 
      timer_charge: reset ? 0 : finalCharge,
