...
export const billingService = {
  async generateBill(
    ...
    try {
      // 1. Update order status and save billing details for later closure
  ...
  async closeTable(
    ...
    try {
      ...
      // Re-calculate totals from order items
      ...
      const parcelCharge = order.parcel_charge || 0;
      const extraCharge = order.extra_charge || 0;
      ...
      // Create the Bill ONLY upon closure
      const billData = {
        order_id: orderId,
        subtotal,
        discount,
        ...
        whatsapp_sent_at: whatsappSent ? new Date() : null
      };

      let existingBill = await Bill.findOne({ order_id: orderId }).session(session);
      if (existingBill) {
        await Bill.updateOne({ order_id: orderId }, billData, { session });
      } else {
        await Bill.create([billData], { session });
      }
  ...
  async getBillWhatsAppLink(billId: string, customPhone?: string): Promise<string> {
    ...
    const displayTable = tNum === 11 ? 'Zomato' : tNum === 12 ? 'Parcel' : `${tNum}`;
    text += `🌿 Table: ${displayTable}${isParcel && tNum !== 12 ? '  📦 PARCEL' : ''}\n${dashes}\n*ITEM${' '.repeat(W - 14)}QTY    AMT*\n${dashes}\n`;
    ...
    text += `${payText}\n${line}\n\n✨ Thank you for dining with us!\n🌿 We hope to see you again soon.\n\n_Cafe Naturaleza — Where Nature Meets Coffee_ 🌿`;
