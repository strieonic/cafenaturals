...
import { ArrowLeft, Plus, Minus, Trash2, Receipt, MessageCircle, RefreshCw, Percent, Printer, Truck, MapPin, Phone, Clock, Play, Square, Timer } from 'lucide-react';
...
export default function OrderEntryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [liveTimerCharge, setLiveTimerCharge] = useState<number>(0);

  const getTimerCharge = useCallback((seconds: number) => {
    const minutes = Math.floor(seconds / 60);
    if (minutes <= 0) return 0;
    if (minutes <= 30) return 75;
    if (minutes <= 75) return 150;
    if (minutes <= 120) return 300;
    
    const extraMinutes = minutes - 120;
    const extraHours = Math.ceil(extraMinutes / 60);
    return 300 + (extraHours * 150);
  }, []);
  ...
  useEffect(() => {
    const updateElapsed = () => {
      ...
      if (order?.timer_start) {
        liveSecs = Math.max(0, Math.floor((Date.now() - new Date(order.timer_start).getTime()) / 1000));
      }
      const totalSecs = (order?.timer_duration_seconds || 0) + liveSecs;
      
      setElapsedTime(totalSecs);
      
      if (totalSecs > 0) {
        setLiveTimerCharge(getTimerCharge(totalSecs));
      } else {
        setLiveTimerCharge(0);
      }
    };
  ...
  }, [order?.timer_start, order?.timer_duration_seconds, getTimerCharge]);
  ...
  const getTableLabel = (num: number | null) => {
    if (num === 11) return 'Zomato';
    if (num === 12) return 'Parcel';
  ...
  const load = useCallback(async (silent = false) => {
    try {
      ...
      if (!silent && tableData?.table_number === 12) {
        setIsParcel(true);
      }
      ...
      if (activeOrder && activeOrder.status === 'billed') {
        ...
        setIsParcel((activeOrder.parcel_charge || 0) > 0 || (tableData?.table_number === 12));
        if (activeOrder.extra_charge) {
          setExtraCharges([{ amount: activeOrder.extra_charge, label: activeOrder.extra_charge_label || '' }]);
        } else {
  ...
  const extraChargeAmount = extraCharges.reduce((s, c) => s + (Number(c.amount) || 0), 0) + (order?.timer_charge || 0) + liveTimerCharge;
  const combinedExtraChargeLabel = [
    ...extraCharges.filter(c => Number(c.amount) > 0).map(c => c.label.trim() || 'Extra Charge'),
    ...((order?.timer_charge || 0) > 0 || liveTimerCharge > 0 ? ['Table Charge'] : [])
  ].join(', ');
  ...
  const startTableTimer = async () => {
    ...
    setBusy(true);
    try {
      const resTime = await db.startTimer(order.id);
      if (resTime) {
        setOrder(prev => prev ? { ...prev, timer_start: resTime } : null);
      }
    } catch (err) {
      console.error(err);
  ...
  const stopTableTimer = async () => {
    if (!order) return;
    setBusy(true);
    try {
      await db.stopTimer(order.id);
      setOrder(prev => prev ? { ...prev, timer_start: null } : null);
    } catch (err) {
      console.error(err);
      showAlert('Error', 'Failed to stop timer');
    } finally {
      setBusy(false);
    }
  };
  ...
  const generateBill = async () => {
    ...
    setBusy(true);
    try {
      // If timer is active, stop it first in database
      if (order.timer_start) {
        await db.stopTimer(order.id, liveTimerCharge);
      }
  ...
  const buildBillText = () => {
    ...
    let text = '';
    text += `${line}\n`;
    text += `    🌿  *CAFE NATURALEZA*\n`;
    text += `         Ishwarpur\n`;
    text += `${line}\n`;
    text += `📅 ${date}  ⏰ ${time}\n`;
    const displayTable = tableNumber === 11 ? 'Zomato' : tableNumber === 12 ? 'Parcel' : `${tableNumber}`;
    text += `🌿 Table: ${displayTable}${isParcel && tableNumber !== 12 ? '  📦 PARCEL' : ''}\n`;
    text += `${dashes}\n`;
    ...
    text += `_Cafe Naturaleza — Where Nature Meets Coffee_ 🌿`;
    return text;
  };
  ...
  return (
    <div className={`bg-[#F9F6F1] flex flex-col min-h-screen ${isBilled ? '' : 'lg:h-screen lg:overflow-hidden'}`}>
      {/* Header */}
      <header className="sticky top-0 z-40 bg-gradient-to-r from-[#FFFDF9] via-[#FFFDF9] to-[#E8D9C5] border-b border-[#E6D8C8] text-[#3E3023] shadow-sm">
        <div className="mx-auto max-w-5xl px-4 py-3 flex items-center gap-3">
          <Link href="/admin" className="p-1.5 hover:bg-[#E8D9C5]/50 rounded-full text-[#3E3023] transition-colors">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <div className="flex-1">
            <h1 className="font-serif text-xl font-bold text-[#3E3023]">{getTableLabel(tableNumber)}</h1>
            <p className="text-[10px] uppercase tracking-widest text-[#6D5B4A] font-sans">
              {isBilled ? 'Bill Generated — Ready to Send' : order ? 'Open Order' : 'No Order'}
            </p>
          </div>
          {/* Delivery badge in header */}
          {deliveryInfo && (
            <div className="flex items-center gap-1 bg-gradient-to-r from-[#C8A97E] to-[#8B6B4A] text-white px-2.5 py-1 rounded-full shadow-sm">
              <Truck className="h-3.5 w-3.5" />
              <span className="text-[10px] font-bold uppercase tracking-wide">Delivery</span>
            </div>
          )}
          {isLocked && (
            <div className="flex items-center gap-1 bg-[#B85C4A] text-white px-2.5 py-1 rounded-full shadow-sm animate-pulse">
          ...
          <div className="flex items-center gap-4">
            {order && !isBilled && (
              <button
                ...
                disabled={busy}
                className="px-3.5 py-1.5 text-[10px] font-bold uppercase tracking-wider bg-[#B85C4A] hover:bg-[#B85C4A]/90 text-white rounded-full transition-colors cursor-pointer shadow-sm hidden sm:block"
              >
                Cancel Order
              </button>
            )}
            {order && !isBilled && (
              <div className="text-right">
                <p className="text-xs text-[#6D5B4A] font-sans">{orderItems.length} item{orderItems.length !== 1 ? 's' : ''}</p>
                <p className="font-mono font-bold text-base text-[#3E3023]">₹{finalTotal}</p>
              </div>
            )}
            {order && !isBilled && (
              <button
                onClick={() => setShowCancelConfirm(true)}
                disabled={busy}
                className="p-2 bg-[#B85C4A] hover:bg-[#B85C4A]/90 text-white rounded-full transition-colors cursor-pointer shadow-sm sm:hidden"
                title="Cancel Order"
              >
                <Trash2 className="h-4 w-4" />
              </button>
            )}
            <div className="border-l border-[#E6D8C8] pl-3 ml-2 text-[#3E3023] flex items-center gap-2">
              <DateTimeDisplay />
              <ViraTechWatermark />
            </div>
          </div>
      ...
      {isBilled ? (
        ...
        <div className="flex-1 flex flex-col lg:flex-row mx-auto max-w-5xl w-full lg:min-h-0">
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden bg-[#FFFDF9] border-t-2 border-[#E6D8C8] shadow-[0_-4px_20px_rgba(0,0,0,0.08)] flex items-stretch mobile-tab-bar">
            <button
              onClick={() => setMobileTab('menu')}
              className={`flex-1 flex flex-col items-center justify-center py-3 gap-1 text-xs font-bold font-sans transition-colors cursor-pointer ${
                mobileTab === 'menu' ? 'text-[#8B6B4A] bg-[#E8D9C5]/50' : 'text-[#6D5B4A]'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex flex-col items-center justify-center py-3 gap-1 text-xs font-bold font-sans transition-colors cursor-pointer relative ${
                mobileTab === 'order' ? 'text-[#8B6B4A] bg-[#E8D9C5]/50' : 'text-[#6D5B4A]'
              }`}
            >
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
              </svg>
              <span>Order{orderItems.length > 0 ? ` (${orderItems.length})` : ''}</span>
              {orderItems.length > 0 && (
                <span className="absolute top-2 right-6 h-4 min-w-4 px-1 bg-[#B85C4A] text-white text-[9px] font-black rounded-full flex items-center justify-center">
                  {orderItems.length}
                </span>
              )}
            </button>
          ...
          {/* LEFT — Menu Panel (full height on mobile when mobileTab==='menu') */}
          <div className={`flex-1 flex flex-col min-w-0 lg:min-h-0 ${
            mobileTab === 'order' ? 'hidden lg:flex' : 'flex'
          } pb-[72px] lg:pb-0`}>
            {/* Category tabs */}
            <div className="sticky top-[60px] z-30 bg-[#F9F6F1] border-b border-[#E6D8C8] select-none">
              <div
                ref={tabsRef}
                ...
                className="flex overflow-x-auto gap-2 px-4 py-3 scrollbar-none cursor-grab active:cursor-grabbing"
              ...
              >
                {categories.map(cat => (
                  <button
                    ...
                    style={{ flexShrink: 0 }}
                    className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap font-sans transition-all border cursor-pointer ${
                      activeCategory === cat.id
                        ? 'bg-gradient-to-r from-[#E8D9C5] to-[#C8A97E] border-[#C8A97E] text-[#3E3023] shadow-sm'
                        : 'bg-[#FFFDF9] border-[#E6D8C8] text-[#6D5B4A] hover:text-[#3E3023] hover:bg-[#E8D9C5]/30'
                    }`}
                  >
                    {cat.name}
                  </button>
          ...
          {/* RIGHT — Order Summary Panel (full height on mobile when mobileTab==='order') */}
          <div className={`w-full lg:w-80 border-t lg:border-t-0 lg:border-l border-border bg-card flex flex-col lg:min-h-0 ${
            ...
            {/* Table Timer Area */}
            {order && (
              <div className="mx-3 mt-3 bg-[#FFFDF9] border border-[#E6D8C8] rounded-2xl p-3 shadow-sm flex flex-col gap-2.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-xs font-bold text-[#3E3023]">
                    <Clock className={`h-4 w-4 ${order.timer_start ? 'text-[#8B6B4A] animate-pulse' : 'text-[#6D5B4A]/50'}`} />
                    <span>Table Timer</span>
                  </div>
                  {order.timer_start && (
                    <span className="inline-flex h-2 w-2 rounded-full bg-[#8B6B4A] animate-pulse" />
                  )}
                </div>

                {!order.timer_start ? (
                  <div className="flex flex-col gap-2">
                    {(order.timer_duration_seconds ?? 0) > 0 && (
                      <div className="flex justify-between items-center bg-[#E8D9C5]/10 border border-[#E6D8C8]/50 rounded-xl px-3 py-1.5">
                        <span className="text-xs text-[#6D5B4A] font-medium">Accumulated Time</span>
                        <span className="text-sm font-mono font-bold text-[#3E3023]">{formatDuration(order.timer_duration_seconds ?? 0)}</span>
                      </div>
                    )}
                    {(order.timer_charge ?? 0) > 0 && (
                      <div className="flex justify-between items-center bg-[#E8D9C5]/10 border border-[#E6D8C8]/50 rounded-xl px-3 py-1.5">
                        <span className="text-xs text-[#6D5B4A] font-medium">Stopped Charge</span>
                        <span className="text-sm font-mono font-bold text-[#8B6B4A]">₹{order.timer_charge}</span>
                      </div>
                    )}
                    <div className="flex gap-2">
                      <button
                        onClick={startTableTimer}
                        disabled={isLocked || busy}
                        className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-gradient-to-r from-[#E8D9C5] to-[#C8A97E] hover:from-[#C8A97E] hover:to-[#8B6B4A] text-[#3E3023] hover:text-white text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shadow-sm disabled:opacity-50"
                      >
                        <Play className="h-3.5 w-3.5" />
                        {(order.timer_duration_seconds ?? 0) > 0 ? 'Resume' : 'Start Timer'}
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col gap-2">
                    <div className="flex justify-between items-baseline">
                      <span className="text-xl font-mono font-black text-[#3E3023] tracking-wide">
                        {formatDuration(elapsedTime)}
                      </span>
                      <span className="text-sm font-bold text-[#8B6B4A] font-mono bg-[#E8D9C5]/30 px-2 py-0.5 rounded-lg border border-[#E6D8C8]/50">
                        +₹{liveTimerCharge}
                      </span>
                    </div>
                    <div className="text-[10px] text-[#6D5B4A] font-sans">
                      Timer is running. It will stop automatically when you generate the bill.
                    </div>
                  </div>
                )}
              </div>
            ...
            {orderItems.length === 0 ? (
              ...
              <>
                {/* Items scroll — full height on mobile (no max-h clipping) */}
                ...
                <div className="px-4 py-4 border-t border-border space-y-3">
                  ...
                  {/* Generate Bill — hidden on mobile (shown in floating bar below) */}
                  <button
                    onClick={generateBill}
                    disabled={busy || orderItems.length === 0}
                    className="hidden lg:flex w-full items-center justify-center gap-2 rounded-xl bg-primary text-primary-foreground px-4 py-3.5 text-sm font-bold font-sans uppercase tracking-wider hover:bg-primary/90 disabled:opacity-50 transition-colors cursor-pointer shadow-sm"
                  >
      ...
      {/* ── MOBILE FLOATING GENERATE BILL BAR ─────────────────────────
           Shown only on mobile (<lg) when on the Order tab and order has items.
           Sits above the bottom tab bar (bottom-[72px]).                   */}
      {!isBilled && orderItems.length > 0 && mobileTab === 'order' && (
        <div className="fixed bottom-[72px] left-0 right-0 z-40 lg:hidden px-4 pb-3 pt-2 bg-gradient-to-t from-[#F9F6F1] via-[#F9F6F1]/95 to-transparent">
          <button
            onClick={generateBill}
            disabled={busy}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#C8A97E] to-[#8B6B4A] text-white px-4 py-3.5 text-sm font-bold font-sans uppercase tracking-wider disabled:opacity-50 transition-all shadow-lg animate-slide-up cursor-pointer"
          >
            {busy ? <RefreshCw className="h-5 w-5 animate-spin" /> : <Receipt className="h-5 w-5" />}
            Generate Bill · ₹{finalTotal}
          </button>
        </div>
      )}

      {/* ── MOBILE "GO TO ORDER" NUDGE ─────────────────────────────────
           When items are added (from menu tab) show a subtle "View Order" pill */}
      {!isBilled && orderItems.length > 0 && mobileTab === 'menu' && (
        <div className="fixed bottom-[72px] right-4 z-40 lg:hidden">
          <button
            onClick={() => setMobileTab('order')}
            className="flex items-center gap-2 bg-[#3E3023] text-[#FFFDF9] px-4 py-2.5 rounded-full text-xs font-bold shadow-xl animate-slide-up cursor-pointer border border-[#8B6B4A]/30"
          >
            <Receipt className="h-4 w-4" />
            View Order ({orderItems.length}) · ₹{finalTotal}
          </button>
        </div>
      )}
