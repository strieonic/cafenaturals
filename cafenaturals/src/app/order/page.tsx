...
function OrderMenuContent() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-[#E8D9C5]/20 backdrop-blur-[0.5px] pointer-events-none z-0" />

      {isLocked && (
        <div className="sticky top-0 bg-[#B85C4A] text-white px-4 py-2 text-center text-[11px] font-bold shadow-md z-50 flex items-center justify-center uppercase tracking-wider">
          ⚠️ Cafe is currently closed for orders.
        </div>
      )}

      {/* Header */}
      <header className="sticky top-0 z-40 bg-gradient-to-r from-[#FFFDF9]/95 via-[#FFFDF9]/95 to-[#E8D9C5]/95 backdrop-blur-md border-b border-[#E6D8C8] text-[#3E3023] shadow-sm">
        <div className="mx-auto max-w-5xl px-4 py-3 flex items-center justify-between">
          <Link href="/" className="p-1.5 hover:bg-[#E8D9C5]/50 rounded-full text-[#3E3023] transition-colors">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <div className="flex-1 text-center">
            <h1 className="font-serif text-lg font-bold text-[#3E3023]">Cafe Naturaleza</h1>
          ...
          <div className="flex items-center gap-2">
            {tableLockedByQR ? (
              <div className="flex items-center gap-1 bg-[#E8D9C5]/50 border border-[#E6D8C8] px-2.5 py-1 rounded-full">
                <span className="text-[9px] font-bold font-sans uppercase tracking-wide text-[#3E3023] opacity-80">🔒 Table {tableNumber}</span>
              </div>
            ) : (
              <button
                onClick={() => setShowTableModal(true)}
                className="bg-gradient-to-r from-[#E8D9C5] to-[#C8A97E] hover:from-[#C8A97E] hover:to-[#8B6B4A] text-[#3E3023] hover:text-[#FFFDF9] px-2.5 py-1 rounded-full text-[9px] font-bold font-sans uppercase tracking-wide transition-all duration-200 shadow-sm"
              >
                Change
              </button>
            )}
            <div className="border-l border-[#E6D8C8] pl-2 ml-1 text-[#3E3023]">
              <DateTimeDisplay />
            </div>
          </div>
        </div>
